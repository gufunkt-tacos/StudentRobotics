...
robot.wait_start()
print(str(distance))
drive_speed_distance(75, distance - 40)
...
while True:
    ...
    if len(markers) != 0:
        for marker in markers:
            print("marker id : ",marker.id, " dist : ", int(marker.position.distance/10), "cms")
            print("angle : ", int(math.degrees(marker.position.horizontal_angle))," degrees")
            if marker.id == 140:
                print("marker id : ",marker.id, " dist : ", int(marker.position.distance/10), "cms")
                if (marker.position.distance/10) > 150:
                    LED_B_red()
                elif ((marker.position.distance/10) >=30) and((marker.position.distance/10) <=150):
                    LED_B_blue()  
                elif ((marker.position.distance/10) <30) :
                    LED_B_green()
    else:
        LED_B_off()
