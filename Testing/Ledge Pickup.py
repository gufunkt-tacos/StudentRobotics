...
)  # , no_powerboard=True) # no_powerboard if not using SR kit
...
wheelspace = (
    37.9  # 37.9 new robot,31.5 tracks,34.60 for old robot,36.75 for 2020 test base
)
...
def drive_speed_distance(speed: int, distance: float) -> bool:
    ...
    if speed > 0:
        while (
            encoder_1() < required_distance_encoder_value
            or encoder_2() < required_distance_encoder_value
        ) and (
            (time.time() - time_started_drive) < drive_timeout_time
        ):  # check drive timeout 15secs
            # print("in drive_speed_distance, encoder1 ",encoder_1())
            pass
    elif speed < 0:
        while (
            (encoder_1()) > (max_encoder - required_distance_encoder_value)
...
def turn_speed_angle(speed: int, angle: float) -> bool:
    ...
    if speed < 0:
        while encoder_2() < angle_encoder * angle and (
            (time.time() - time_started_turn) < turn_timeout_time
        ):  # check drive timeout 10secs:
            pass
        motor_stop()
        time.sleep(0.1)
    elif speed > 0:
        while encoder_1() < angle_encoder * angle and (
            (time.time() - time_started_turn) < turn_timeout_time
        ):  # check drive timeout 10secs:
            pass
        motor_stop()
...
def Arm_tilt(position):  # Servo port [1] MS24, 270 degree servo, 20kg
    ...
    servo_angle = (
        arm_angle / 100
    ) * 0.75  # + camera_servo_offset # converts angle to range of +/- 1.0
    ...
    time.sleep(0.1)  # allow time to move
...
def camera_pan(
    angle,
):  # Camera servo = servo [0]. Servo Board= "0LX2M". Resolution ~ 0.5 degrees
    global cam_angle
    camera_servo_offset = (
        camera_servo_offset_value / 100
    ) * 1.5  # value of 1 degree = 0.02
    servo_angle = (
        angle / 100
    ) * 1.5 + camera_servo_offset  # converts angle to range of +/- 1.0
    if servo_angle >= 1:  # Set max range for servo used and SR max allowed value
        servo_angle = 1
    if servo_angle <= -1:  # Set max range for servo used and SR max allowed value
        servo_angle = -1
...
def Doors_close_LR():
    ...
    time.sleep(0.5)  # then close RH door
    RH_door(90)
    time.sleep(0.5)
...
def Doors_close_RL():
    ...
    time.sleep(0.5)  # then close LH door
    LH_door(-93)
    time.sleep(0.5)
...
def find_objects(object_type: ObjectType) -> list[Object] | None:
    ...
    for marker in markers:
        ...
        if ((robot_distance**2) - ((camera_vertical_height - 0) ** 2)) <= 0:
            robot_distance = 1000
        robot_distance_corrected = round(
            math.sqrt((robot_distance**2) - ((camera_vertical_height - 0) ** 2))
            - camera_horizontal_offset,
            0,
        )

        h_uncorrected_robot_angle = round(
            math.degrees(marker.position.horizontal_angle)
        )
        ...
        objects.append(
            Object(marker.id, robot_distance_corrected, h_robot_angle, v_robot_angle)
        )
...
if port != "not detected":  # i.e USB-ISS has been detected
    ser = serial.Serial(
        port, 115200
    )  # Set up serial communications (ser) with USB-ISS module
    ...
    print(
        "USB-ISS adaptor OK, but if an error is now detected then there is a problem talking to the MD25"
    )
...
def confirm_grip() -> bool:
    """
    Returns true when box has been confirmed to held by the suction cup using the pressure sensor.
    """
    # not sure how to interface with pressure sensor
    pressure = int(robot.arduino.command("z"))
    if pressure < 50:
        return True
    return False
...
def go_home():
    ...
    arena_markers = find_objects("arena_marker")
    ...
    if marker_found == True:
        turn_speed_angle(5, closest_token_angle)
        ...
        if marker_found == True:
            turn_speed_angle(5, closest_token_angle)
            drive_speed_distance(30, closest_token_dist)
            turn_speed_angle(5, 90 * sign(closest_token_angle))
        else:
            drive_speed_distance(30, 25)
...
def navigate_obstacle():
    Doors_wedge()
    right_sonar = right_front_sonar()
    left_sonar = left_front_sonar()
    if right_sonar < 30 or left_sonar < 30:
        ...
        valid_tokens = find_objects("acid_tokens")
        print(valid_tokens)
        if valid_tokens[0] == True:
            print(len(valid_tokens[2]))
            for i in range(0, len(valid_tokens[2])):
        ...
        if closest_token_dist > 50:
            turn_speed_angle(32 * sign(right_sonar - left_sonar), 90)
            drive_speed_distance(20, 50)
            turn_speed_angle(32 * -sign(right_sonar - left_sonar), 90)
