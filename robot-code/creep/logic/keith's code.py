...
camera_servo_offset_value = -2 # +ve offset anti-clock
#...............................................................................
...
# Drive times out after a calculated time is exceeded (speed, distance based)
def drive_speed_distance_objchk(speed, distance, object_detect_range):
    # nominal speed of robot at speed 32 = 25cm/s (at speed 32)
    # adjust for accel/decel times etc and add a 1 second margin
    if speed != 0 :
        drive_timeout_time = (((distance * 32) / 22) /abs(speed)) +2
    ...
    if speed > 0 :
       ...
       #distance_driven = (encoder_1() * wheel_diameter * 3.142)/360
            range_detected_front_left = left_front_sonar()
            move = True
            ...
            range_detected_front_right = right_front_sonar()
        ...
        #while (encoder_1()-0) > (max_encoder - required_distance_encoder_value):
            ...
            range_detected_rear = rear_sonar()
            move = True
    ...
    if speed >0 :
        distance_driven = round(((encoder_1() * wheel_diameter * 3.142)/360),2)
        if object_detected == True:
            print ("object at front left ", range_detected_front_left)
            print ("object at front right", range_detected_front_right)
        
    elif speed < 0:
        distance_driven = round((((max_encoder - encoder_1()) * wheel_diameter * 3.142)/360),2)
        if object_detected == True:
...
def Arm_tilt(position):  # Servo port [1] MS24, 270 degree servo, 20kg
    ...
    if (position) == "LEVEL":
        ...
        angle = 0     
    servo_angle = (arm_angle/100)*.75 #  converts angle to range of +/- 1.0
...
def sucker_height():
    ...
    ir_distance = round(ir_distance,1)
    return ir_distance
#_______________________________________________________________________________
# KCH LED routines
def LED_B_red():
...
def RH_door(angle):  # Servo port [7] MS24, 270degree servo, 20kg
    ...
    servo_angle = (angle/100)*.75 #converts angle to range of +/- 1.0
    if servo_angle >= 1:  # Set max range for servo used and SR max allowed value
        servo_angle = 1
    if servo_angle <= -1: # Set max range for servo used and SR max allowed value
        servo_angle = -1
...
def find_objects(object):
    ...
    angle_correction = 0
    camera_vertical_height = 44.5
    camera_horizontal_offset = 5.0 # to front doors
...
camera_pan(90)   # centre camera may wish to change this to 90
LH_door(-93)    # close LH door
RH_door(90)     # close RH door
...
# Look for an base, drive & get, go to acid token position, turn 90, back up then get platform base
while True:
    base_tokens_found, base_tokens_ids, base_tokens_dist, base_tokens_angle = find_objects("base_tokens")
    while base_tokens_found ==False:
        base_tokens_found, base_tokens_ids, base_tokens_dist, base_tokens_angle = find_objects("base_tokens")
        print(base_tokens_found, base_tokens_ids, base_tokens_dist, base_tokens_angle)
        
    if base_tokens_found == True:
        angle = base_tokens_angle[0]
    ...
    if base_tokens_found == True: 
        ...
        turn_speed_angle(turnspeed, angle)
        time.sleep(.5)
    ...
    if base_tokens_found == True:
        ...
        Doors_open()
        time.sleep(1)
        drive_speed_distance(48,base_tokens_dist[0]+25)
        Doors_close()
        
    # Look for an acid token

    acid_tokens_found, acid_tokens_ids, acid_tokens_dist, acid_tokens_angle = find_objects("acid_tokens")
    if acid_tokens_found == True:
        print(acid_tokens_found, acid_tokens_ids, acid_tokens_dist, acid_tokens_angle)
    
        angle = acid_tokens_angle[0]
        print ("Angle to turn = ", angle)    
    ...
    if acid_tokens_found == True:
        ...
        #Drive to push away acid token and end up at middle of plinth
        
        time.sleep(1)
        Doors_wedge()
        time.sleep(1)
        drive_speed_distance(32,acid_tokens_dist[0]+35)
        Doors_close()
    
    #turn towards platform
    turn_speed_angle(-16,90)
    #reverse away from platform
    drive_speed_distance(-16,70)

    print("Now looking for base token on plinth")
    while base_tokens_found == False: 
        base_tokens_found, base_tokens_ids, base_tokens_dist, base_tokens_angle = find_objects("base_tokens")
        print(base_tokens_found, base_tokens_ids, base_tokens_dist, base_tokens_angle)
    ...
    if base_tokens_found == True:
        ...
        print ("Turn angle = ",angle)    
        turn_speed_angle(turnspeed, angle)
        dist = base_tokens_dist[0] - 40
        if dist <= 0:
            drive_speed_distance(8,0)
        time.sleep(.2)
    ...
    if base_tokens_found == True:
        ...
        left_sonar_distance = left_front_sonar()
        right_sonar_distance = right_front_sonar()
        #drive until plinth detected at 8cm L or R sonar
               
        drive_speed_distance_objchk(16,120,6)
        motor_stop
        ...
        Arm_Stop()  
        #vacuum on
        VacValve("GRIP") # Allows suction
        VacPump(1)
        
        #robot.motor_boards["SR0VJ1K"].motors[0].power = 1
    time.sleep(1)
    #tilt arm down
    vacPressure = int(robot.arduino.command("z"))
    Arm_tilt_down()
    ...
    #retract arm
    #robot.motor_boards["SR0VJ1K"].motors[1].power = 1.0
    Arm_Retract(1) # full speed. Takes around 7 seconds 
    time.sleep(7)
    
    #vacuum off
    #robot.motor_boards["SR0VJ1K"].motors[0].power = 0
    VacValve("VENT") # releases vacuum
    VacPump(0)
    ...
    drive_speed_distance_objchk(-8,50,20)
        
    startup_jingle()    
        #while left_sonar_distance >8:
        #    drive_both(8,8)
        #    left_sonar_distance = left_front_sonar()
        #motor_stop() 
